...
// const newArr = 0
// const sum = arr.reduce((accumulator , currentValue) => accumulator + currentValue, newArr);
// return sum;
...
const returnNames = (arr) => {
  // Solution code here...
  return arr.map((starWarsData)=> {
    return starWarsData.name;
  });
};
// const newArr = arr.reduce((acc, starWarsData) => {
//   acc.push(starWarsData.name);
//   return acc;
// }, []);
// return newArr
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  return arr.reduce((count, current) => isPrime(current) ? count + 1 : count, 0);
};
