...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let first = recipe.ingredients[i].indexOf(' ');
    let food = recipe.ingredients[i].slice(first + 1);
    let second = food.indexOf(' ');
    if (second !== -1) {
      food = food.slice(second + 1);
    }
    result.push(food)
  }
  return result;
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    const splitIngredients = recipe.ingredients[i].split(' ');
    result.push(splitIngredients.slice(2).join(' '))
  }
  return result;
};
