...
const maxInArray = (arr) => {
  // Solution code here...
  const maxVal = arr.reduce((max, num) => Math.max(max,num));
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  const foundCharacter = arr.find((person) => person.name === character)
  if (foundCharacter) {
    return Object.values(foundCharacter).includes(foundCharacter.children);
  }
  return false;
};
