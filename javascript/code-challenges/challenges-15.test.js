...
const screenForNames = (arr) => {
  ...
  const filterName = arr.filter((name) => regex.test(name));
  return filterName;
}
...
const toTitleCase = (arr) => {
  // Solution code here...
  const title = arr.map((array) => {
    const firstChar = array.charAt(0).toUpperCase();
    const restoreString = array.slice(1);
    return firstChar + restoreString;
  })
  return title;
};
...
let biggerThanLuke = (arr) => {
  ...
  const names = arr
  .filter((character) => character.mass > lukeMass)
  .map((character) => character.name);
  console.log(names);
  const combineNames = names.join(' - ')
