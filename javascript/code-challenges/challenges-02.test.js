...
const mapTwoToThe = (arr) => {
  // Solution code here...
  const newAr = arr.map(num => Math.pow(2,num));
  return newAr;
};
...
const charCode = (arr) => {
  // Solution code here...
  const newCh = arr.map(word => arr.charCodeAt(newCh));
  return newCh;
};
